...
from app.schemas.memberManagements import (
...
async def toggle_member(room_id: str, member_id: str, data: ToggleAccessSchema, db: AsyncIOMotorDatabase = Depends(get_db)):
...
@router.put("/update/{member_id}", response_model=MemberSchema)
async def update_member(member_id: str, data: MemberUpdateSchema, db: AsyncIOMotorDatabase = Depends(get_db)):
    updated = await crud.update_member(db, member_id, data)
    if updated is None:
        raise HTTPException(status_code=404, detail="Member not found or invalid id")
    return updated
...
async def remove_member(member_id: str, db: AsyncIOMotorDatabase = Depends(get_db)):
    ...
    if isinstance(result, dict) and result.get("error") == "invalid_id":
